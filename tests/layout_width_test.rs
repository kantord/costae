...
/// A flex-col root WITHOUT w-full shrinks to content (width=0 with no children).
/// Flex containers don't auto-expand like block elements in Taffy.
#[test]
...
/// With w-full, a flex-col root fills the viewport at DPR=1.0.
#[test]
...
/// With w-full, a flex-col root fills the physical viewport at DPR=1.46.
#[test]
...
/// A child with w-full inside a padded root fills the root's content box.
#[test]
fn padded_root_children_with_w_full_fill_content_box() {
    ...
    let expected_padding = 0.75 * 16.0 * 1.46 * 2.0; // 0.75rem * font_size * dpr * 2
...
/// The rendered pixel extent of a child matches the Taffy layout (no hidden DPR scaling).
#[test]
fn rendered_child_width_matches_layout_at_dpr_1_46() {
    ...
    let bgrx = render_frame(Some(layout), &global, 146, 50, 1.458);

    // Find leftmost and rightmost blue pixel in row 0 (BGRX: blue=px[0], red=px[2])
    let row0 = &bgrx[..146 * 4];
    ...
    // Without root padding, blue child should fill nearly the full viewport
    assert!(
        blue_width >= 140,
