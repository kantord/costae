...
fn render_frame_respects_width_parameter() {
    let bgrx = render_frame(None, &GlobalContext::default(), 100, 50, 1.0);
    assert_eq!(bgrx.len(), (100 * 50 * 4) as usize);
}

#[test]
fn render_frame_respects_different_width() {
    let bgrx_200 = render_frame(None, &GlobalContext::default(), 200, 50, 1.0);
    let bgrx_400 = render_frame(None, &GlobalContext::default(), 400, 50, 1.0);
...
fn render_frame_with_layout_returns_correct_size() {
    ...
    let bgrx = render_frame(Some(layout), &GlobalContext::default(), 100, 200, 1.0);
